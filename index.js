...
process.on('SIGINT', async () => {
    ...
    console.log();
    console.error('Operation Terminated: Closing database and exiting...');
    ...
    const message = await interaction.channel.messages.fetch(data.Msg);
      ...
      if (!message.hasThread) {
        const thread = await message.startThread({
            name: `Discussion on Suggestion`, // Customize the thread name as needed
            reason: 'Community discussion on suggestion'
        });

        await thread.send({ embeds: [message.embeds[0]] });

        const threadReplyEmbed = new EmbedBuilder()
            .setColor(0x18e1ee)
            .setTitle('Discuss the Suggestion')
            .setDescription('Feel free to discuss this suggestion with the community!')
            .setTimestamp();
